...
# prefix =  "/home/ubuntu/AsynNet/acs/script/client/log/client"
prefix = "/home/ubuntu/AsynNet/acs/script/client/log/client"
class BKRLogger:
    def __init__(self, n, prefix, name):
        self.n = n
        self.prefix = prefix
        self.name = name
        self.region_latency = {}
    ...
    def read_info(self,):
        for i in range(self.n):
            filename = self.prefix + str(i) + '/' + self.name
            txs = 0
            latency = 0
...
def terminal_plot(series, delta):
    print(f"                                     Throughput Time Series Graph")
    x = [t * delta for t in range(len(series))]
    fig = tpl.figure()
    # fig.ylim(y=0)
    fig.plot(x, series, width=100, ylim = [0, 700], height=15)
    fig.show()
...
if __name__ == "__main__":
    ...
    d = BKRLogger(total_nodes, prefix, name)
